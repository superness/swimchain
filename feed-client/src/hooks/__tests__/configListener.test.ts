...
/**
 * Drives feed-client's REAL config-handover listeners — not the pure trust
 * functions (those are covered by @swimchain/frontend's own configTrust
 * tests). This proves feed-client actually WIRED isConfigMessageTrusted +
 ...
 * listener body to the old `ALLOWED_ORIGINS.some(a => origin.startsWith(a))`
