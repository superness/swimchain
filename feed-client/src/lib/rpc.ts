...
export class SwimchainRpc {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error('[RPC] Error response body:', errorBody);
        } catch {
          console.error('[RPC] Could not read error response body');
        }
  ...
  /**
   * Get details of a sponsorship offer including pending claims
   */
  ...
  /**
   * Create a new sponsorship offer
   */
  ...
  /**
   * List the current user's sponsorship offers
   */
  ...
  /**
   * Check if the user has a pending sponsorship claim
   */
   ...
   * Uses the node's get_user_posts RPC method; returns the same
   * ContentResult shape as listSpaceContent.
   */
...
// For browser testing without local node, use TESTNET_SEED_SF or TESTNET_SEED_NYC
