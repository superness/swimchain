...
// LRU cache limit to prevent memory leaks (P-03)
...
function evictOldestEntries(): void {
  ...
  // Convert to array, sort by timestamp (oldest first), delete excess
  const entries = Array.from(memoryCache.entries());
  entries.sort((a, b) => a[1].timestamp - b[1].timestamp);

  const toDelete = entries.slice(0, entries.length - MEMORY_CACHE_MAX_ENTRIES);
