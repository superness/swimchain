//! `app.json` manifest parsing for launcher apps.
//!
//! This is intentionally a copy of `launcher-apps/app-shell/src-tauri/src/manifest.rs`:
//! the desktop launcher (this crate) and the per-app shell both need to read the same
//! `app.json` contract. Deduplicating into a shared crate is deferred to Phase 2.
...
mod tests {
    ...
    fn parses_full_manifest_and_defaults_single_instance() {
        ...
        assert!(
            m.single_instance,
            "single_instance defaults to true when omitted"
        );
    }
    ...
    fn rejects_missing_required_fields() {
        assert!(
            parse_manifest(r#"{"name":"x","exec":"y"}"#).is_err(),
            "missing id"
        );
        assert!(
            parse_manifest(r#"{"id":"x","exec":"y"}"#).is_err(),
            "missing name"
        );
        assert!(
            parse_manifest(r#"{"id":"x","name":"y"}"#).is_err(),
            "missing exec"
        );
    ...
    fn feed_manifest_file_parses() {
        let json = include_str!("../../../launcher-apps/feed/app.json");
        let m = parse_manifest(json).unwrap();
        assert_eq!(m.id, "feed");
        assert_eq!(m.exec, "feed-app.exe");
    }
