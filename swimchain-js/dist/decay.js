 ...
 * ```
 */
export function calculateDecay(createdAtSecs, lastEngagementSecs, nowSecs) {
    const now = nowSecs ?? Math.floor(Date.now() / 1000);
    const wasm = getWasm();
    const state = wasm.calculate_decay(BigInt(createdAtSecs), BigInt(lastEngagementSecs), BigInt(now));
    const result = {
 ...
 * @returns Decay state information
 */
export function calculateDecayWithHalfLife(createdAtSecs, lastEngagementSecs, nowSecs, halfLifeSecs) {
    const wasm = getWasm();
    const state = wasm.calculateDecayWithHalfLife(BigInt(createdAtSecs), BigInt(lastEngagementSecs), BigInt(nowSecs), BigInt(halfLifeSecs));
    const result = {
        currentHeat: state.currentHeat,
        isDecayed: state.isDecayed,
        isProtected: state.isProtected,
 ...
 */
export function getDecayConstants() {
    const wasm = getWasm();
    return {
 ...
 */
export function survivalProbability(halfLivesElapsed) {
 ...
 */
export function halfLivesFromProbability(probability) {
 ...
 */
export function isDecayedAtProbability(probability) {
 ...
 */
export function formatDecayState(state) {
