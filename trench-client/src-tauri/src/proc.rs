...
//!
//! The shell itself is a GUI (windows_subsystem = "windows") binary, but the node
//! binary `sw.exe` is a *console* subsystem binary. When a GUI process spawns a
//! console child, Windows allocates a fresh console for it — a blank command
//! prompt sitting next to the game window. The node's stdout/stderr already go to
//! the log file, so that console shows nothing and does nothing except confuse
//! players. `CREATE_NO_WINDOW` suppresses it; stdio pipes keep working as before.
//!
