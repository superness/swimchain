/**
 * Pure identity-mode selection for wiki-client.
 *
 * Kept free of any React / SDK / WASM imports so it can be reasoned about and
 * unit-tested in isolation, and reused by {@link ./useWikiIdentity}.
 *
 * Mirrors feed-client's `identityMode.ts` (PR #64) and chat/search so every
 * bundled client adopts the node identity the same way when embedded in the
 * desktop shell.
 */

/** Which identity source wiki should use. */
...
/**
 * Decide whether wiki adopts the node identity or the browser keypair.
 *
