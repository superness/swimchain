...
export const StartDmModal:FC<P>=({onClose})=>{const{identity}=useIdentityContext();const{sendRequest,activeDms}=useDm();const[pk,setPk]=useState('');const[s,setS]=useState(false);const[err,setErr]=useState<string|null>(null);const[ok,setOk]=useState(false);
const sub=useCallback(async(e:React.FormEvent)=>{e.preventDefault();const v=pk.trim().toLowerCase();if(!v)return;if(v===identity?.publicKey?.toLowerCase()){setErr('Cannot DM self');return}if(activeDms.some(d=>d.otherPk.toLowerCase()===v)){setErr('Already exists');return}setS(true);setErr(null);const r=await sendRequest(v);if(r){setOk(true);setTimeout(onClose,1e3)}else setErr('Failed');setS(false)},[pk,identity,activeDms,sendRequest,onClose]);
