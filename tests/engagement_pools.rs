...
use swimchain::content::pool::{
    compute_pool_pow_target, CompletionResult, EngagementPool, PoolContribution,
    PoolError, PoolId, PoolManager, PoolStatus,
};
...
fn test_pool_id_differs_for_different_content() {
    ...
    assert_ne!(id1, id2, "Different content should produce different pool IDs");
}
...
fn test_single_contributor_completion() {
    ...
    manager.add_contribution(pool_id, contribution, 5000, &config).unwrap();

    let result = manager.check_completion(pool_id).unwrap();
    match result {
        CompletionResult::Completed { total_pow, contributor_count, .. } => {
...
fn test_multi_contributor_completion() {
    ...
    let result = manager.check_completion(pool_id).unwrap();
    match result {
        CompletionResult::Completed { total_pow, contributor_count, contributors } => {
            assert_eq!(total_pow, 60);
...
fn test_sybil_resistance_equivalence() {
    ...
    m1.add_contribution(pool_id1, single_contribution, 5000, &config).unwrap();
    ...
    for i in 0..60u8 {
        ...
        m2.add_contribution(pool_id2, contribution, 5000, &config).unwrap();
    }
...
fn test_same_contributor_multiple_times() {
    ...
    manager.add_contribution(pool_id, c1, 5000, &config).unwrap();
    manager.add_contribution(pool_id, c2, 5000, &config).unwrap();
...
fn test_minimum_contribution_enforced() {
    ...
    assert!(matches!(
        result,
        Err(PoolError::ContributionTooSmall { provided: 0, minimum: 1 })
    ));
}
...
fn test_get_pool_info_for_content_returns_highest() {
    ...
    manager.add_contribution(pool1, make_contribution([1u8; 32], 10, 1000, target1), 5000, &config).unwrap();
    manager.add_contribution(pool2, make_contribution([2u8; 32], 30, 1000, target2), 5000, &config).unwrap();
    manager.add_contribution(pool3, make_contribution([3u8; 32], 20, 1000, target3), 5000, &config).unwrap();
