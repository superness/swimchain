...
let mockStoredIdentityReturn;
let mockStoredKeypairReturn;
...
describe('useGameIdentity', () => {
    ...
    describe('browser mode (standalone tab)', () => {
        ...
        beforeEach(() => {
            ...
            browserSignSpy = vi.fn((_m) => new Uint8Array([9, 9, 9]));
            setAuthSpy = vi.fn();
        ...
        it('resolves identity from localStorage, pushes setAuth with the browser keypair, and never calls sign_message', async () => {
            ...
            expect(rpcCallSpy.mock.calls.some(([method]) => method === 'sign_message')).toBe(false);
            unmount();
        });
    ...
    describe('node mode (embedded, shell shared a node identity)', () => {
        ...
        beforeEach(() => {
            ...
            saveIdentitySpy = vi.fn();
            clearIdentitySpy = vi.fn();
            decoySignSpy = vi.fn((_m) => new Uint8Array([7, 7, 7]));
            setAuthSpy = vi.fn();
            rpcCallSpy = vi.fn(async (method) => {
                if (method === 'get_identity_info') {
        ...
        it('resolves identity from get_identity_info, never PUSHES a signing setAuth (anti-recursion), and signs via sign_message', async () => {
            ...
            const signMessageCalls = rpcCallSpy.mock.calls.filter(([method]) => method === 'sign_message');
...
//# sourceMappingURL=useGameIdentity.test.js.map