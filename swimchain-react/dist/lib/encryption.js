 ...
 */
function getCacheKey(passphrase, salt) {
 ...
 */
export async function encryptContent(content, passphrase) {
    const encoder = new TextEncoder();
 ...
 */
export async function decryptContent(encryptedContent, passphrase) {
 ...
 */
export async function encryptPost(title, body, passphrase) {
 ...
 */
export async function decryptPost(encryptedBody, passphrase) {
 ...
 */
export async function encryptMedia(data, passphrase) {
 ...
 */
export async function decryptMedia(encryptedData, passphrase) {
    try {
 ...
 */
export async function encryptWithSpaceKey(content, spaceKey) {
    const encoder = new TextEncoder();
    // Generate random IV
    const iv = crypto.getRandomValues(new Uint8Array(IV_LENGTH));
 ...
 * @returns Decrypted plain text, or null if decryption fails
 */
export async function decryptWithSpaceKey(encryptedContent, spaceKey) {
 ...
 */
export async function encryptPrivatePost(title, body, spaceKey) {
    const fullContent = `${title}\n\n${body}`;
 ...
 */
export async function decryptPrivatePost(encryptedBody, spaceKey) {
 ...
 */
export async function encryptPrivateMedia(data, spaceKey) {
    const bytes = data instanceof Uint8Array ? data : new Uint8Array(data);
    const plainBuffer = new ArrayBuffer(bytes.length);
    new Uint8Array(plainBuffer).set(bytes);
    // Generate random IV
 ...
 */
export async function decryptPrivateMedia(encryptedData, spaceKey) {
    try {
        const combined = encryptedData instanceof Uint8Array
            ? encryptedData
            : new Uint8Array(encryptedData);
 ...
 */
export async function encryptSpaceName(name, spaceKey) {
    const encoder = new TextEncoder();
    const iv = crypto.getRandomValues(new Uint8Array(IV_LENGTH));
    const key = await importSpaceKey(spaceKey);
 ...
 */
export async function decryptSpaceName(encryptedName, spaceKey) {
    try {
