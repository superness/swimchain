...
export const TRUSTED_PARENT_ORIGINS = new Set([
    'tauri://localhost', // Tauri v1 shell
    'http://tauri.localhost', // Tauri v2 shell
    'https://tauri.localhost',
]);
export function isConfigMessageTrusted(event, ctx) {
    if (event.source == null || event.source !== ctx.parentWindow)
        return false; // event.source === window.parent
    const origin = event.origin;
    if (origin && origin === ctx.selfOrigin)
        return true; // exact same-origin (the embed case), never ""
