 ...
 * they're in.
 */
export function useGameIdentity(): UseGameIdentityResult {
  // ---- Mode selection ----
  ...
  const [parentConfig, setParentConfig] = useState<ParentRpcConfig | null>(() => getParentConfig());
  ...
  const {
    ...
    isLoading: browserIdentityLoading,
    saveIdentity: saveBrowserIdentity,
    clearIdentity: clearBrowserIdentity,
  } = useStoredIdentity();
  const {
    keypair: browserKeypair,
    publicKey: browserPublicKeyBytes,
    publicKeyHex: browserPublicKeyHex,
    sign: browserSignSync,
  } = useStoredKeypair();
  ...
  const identity = useMemo<GameIdentity | null>(() => {
    if (mode === 'node') {
  ...
  const saveIdentity = useCallback(
    (next: StoredIdentity) => {
      if (mode === 'node' || mode === 'pending') return;
