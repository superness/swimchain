...
type Listener = (config: ParentRpcConfig | null) => void;
...
vi.mock('../lib/parentConfig', () => ({
  ...
  subscribeParentConfig: (fn: Listener) => {
...
function Probe() {
  ...
  return <div data-testid="probe">{connected ? 'connected' : connecting ? 'connecting' : 'idle'}</div>;
}
...
function fakeFetch() {
  return vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => fakeNodeInfoResponse());
}
...
describe('RpcProvider — iframe config-already-arrived at mount (review finding #2)', () => {
  ...
  it('connects to the parent endpoint (not LOCAL_TESTNET) using the singleton already present at mount', async () => {
    ...
    expect((init as RequestInit).headers).toMatchObject({ Authorization: 'Basic parent-cookie' });
