...
**Date:** 2026-07-29 · **Status:** awaiting operator rulings · **Spec:** `2026-07-28-surf-channel-app-design.md` §3.3, §3.4, §5 B, §9

Every fact below was verified in the worktree tonight (file:line in parentheses). Each dial ends with a RULING line for you to fill. Nothing here is consensus — per the fold-rules law, everything below is node-policy or client-policy and stays tunable.
...
**Signing wrinkle (needs your eyes):** the shell holds no key material — the seed never leaves the node (§2.5). To sign `engage:{content_id}:{nonce}:{timestamp}` (`methods.rs:3918-3937`) the shell must call the node's `sign_message` RPC over loopback+cookie. §2.4 bans `sign_message` from any *channel grant*; the shell is not a channel and is baked trusted code, so I read this as within the letter and spirit for the A/B era — Phase D's purpose-scoped signing replaces it. But it's a security-posture call and it's yours.
...
**RULING B2 (N / K / weight / 24h-policy / sign_message-via-shell ok?):** ______
...
**Proposed:** ruled out for B — "desktop-only, revisit at E." The dial stays feed/wiki/reef until C adds the rest of the baked fleet.
...
**Proposed:** acquisition's follow-set becomes top-N (N=3) spaces from `list_spaces` filtered to `class == "social"` (or B1's RPC once it exists, preferring engagement recency), with the current hardcoded three as the fallback when the listing is empty (true first run before any sync — the static covers that wait regardless).

**RULING B5:** ______

## B6 — Dead Air thresholds (§3.3)

**Proposed** (all client policy, tuned against the 7-day half-life): last engagement <2d = alive, no card; 2–5d = test card with `LAST SIGNAL: N DAYS AGO`, flare available; >5d = `THIS CHANNEL IS DYING`, flare available; flare's target = the space's most recent surviving item via the B1 listing (spec-defined fallback text when nothing is retrievable). Flare = `request_content` + one engage on arrival, same signing path as B2.
