...
- **B4** feed: wire followed-user posts via existing `get_user_posts`; remove fabricated local-space fallback. (M) — PR #7 open on GitHub
- **B5** search: delete MacroRegimeCard; fix dead footer routes; unify deep-link target. (S) **✔ DONE** — merged to main (commit 991b0cc)
