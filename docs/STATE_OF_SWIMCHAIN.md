...
- **B7** bridge: ship/document IRC WebSocket proxy (**DONE** — `proxy/irc-proxy.js` with README); queue (don't drop) messages during mining (**DONE** — message queue in `BridgeEngine` with `drainQueue()`); thread inbound as replies (**DONE** — `resolveThreadParent()` maps channel/room to recent bridged post within 30-min window, submits via `submitReply`). (M)
