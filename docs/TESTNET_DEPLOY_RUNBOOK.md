...
- **CLI→node RPC needs a cookie.** A bot data dir with no running node (e.g. `/root/dispatch-bot`) has no `.cookie`, so `sw --data-dir X <cmd>` fails "Failed to read RPC cookie." Share the running node's cookie: `cp /var/lib/swimchain-testnet/.cookie /root/dispatch-bot-testnet/.cookie`. Then the CLI signs with X's identity but authenticates to the seed node's RPC (`127.0.0.1:19736`).
