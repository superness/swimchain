...
//!
//! Questions to answer:
//! 1. Given N viewers, what's content availability after X days?
//! 2. How does cache size affect network health?
//! 3. What's the relationship between engagement and persistence?
...
struct User {
    id: u64,
    cache: VecDeque<u64>,           // content IDs in LRU order
    cache_bytes: u64,               // current cache usage
    max_cache_bytes: u64,           // cache limit
    own_content: HashSet<u64>,      // content this user created (always stored)
    online_probability: f64,        // chance of being online at any moment
}
...
impl User {
    fn new(id: u64, max_cache_mb: u64) -> Self {
        Self {
            ...
            online_probability: 0.3, // 30% online at any moment
        }
    }

    /// View content - adds to cache with LRU eviction
    fn view_content(&mut self, content_id: u64, content_size: u64) {
        ...
        while self.cache_bytes > self.max_cache_bytes {
            if let Some(evicted) = self.cache.pop_back() {
                // In real system, we'd look up size. Assume average.
                self.cache_bytes = self.cache_bytes.saturating_sub(content_size);
            }
        }
    }

    /// Check if user has content (own content + cache)
...
impl Network {
    ...
    /// User views content (simulates fetch + cache)
    ...
    /// Check content availability (can we find a seeder?)
...
struct SimulationResult {
    ...
    availability_by_views: HashMap<u64, (u64, u64)>,  // views -> (available, total)
    avg_seeders_by_views: HashMap<u64, f64>,
}

/// Run the availability simulation
fn run_availability_simulation(
    num_users: u64,
    cache_mb: u64,
    posts_per_user: u64,
    ...
    // Simulate posting and viewing over time
    ...
    for day in 0..days_to_simulate {
        ...
        for _ in 0..posts_per_day {
            ...
            let size = if is_image { image_size_bytes } else { text_size_bytes };

            let content_id = network.post_content(user_id, size);

            // Random views based on engagement distribution
            // Use power law: most posts get few views, some get many
            let view_count = {
    ...
    for content in network.content.values() {
        ...
        // Bucket views: 0, 1-5, 6-20, 21-100, 100+
        ...
        seeders_by_views.entry(view_bucket).or_default().push(seeder_count);
    }

    // Calculate average seeders
    let avg_seeders: HashMap<u64, f64> = seeders_by_views.iter()
        .map(|(&bucket, seeders)| {
...
fn main() {
    ...
    let scenarios = [
        ("Small Network (100 users)", 100, 500, 50, 30),
    ...
    for (name, users, cache_mb, posts_per_user, days) in scenarios {
        println!("┌──────────────────────────────────────────────────────────────────────────┐");
        println!("│ Scenario: {:<62}│", name);
        println!("│ Users: {:<6} | Cache: {}MB | Posts/user: {:<4} | Days: {:<3}            │",
                 users, cache_mb, posts_per_user, days);
        println!("├──────────────────────────────────────────────────────────────────────────┤");

        let result = run_availability_simulation(
            users,
            cache_mb,
            posts_per_user,
            0.20,              // 20% images
            2_000_000,         // 2MB average image
            2_000,             // 2KB text
            (users as f64 * 0.05).max(5.0),  // 5% of network views on average
            days,
        );

        let availability_pct = result.available_content as f64 / result.total_content as f64 * 100.0;

        println!("│ Overall: {}/{} content available ({:.1}%)                    │",
                 result.available_content, result.total_content, availability_pct);
        println!("│                                                                          │");
        println!("│ Availability by engagement:                                              │");

        let buckets = [0, 5, 20, 100, 1000];
        let bucket_names = ["0 views", "1-5 views", "6-20 views", "21-100 views", "100+ views"];
        ...
        for (i, &bucket) in buckets.iter().enumerate() {
            if let Some((avail, total)) = result.availability_by_views.get(&bucket) {
                ...
                println!("│   {:12}: {:>4}/{:<4} ({:>5.1}%) - avg {:.1} seeders             │",
    ...
    for (size_name, size_bytes) in [("500KB", 500_000), ("2MB", 2_000_000), ("5MB", 5_000_000)] {
        let result = run_availability_simulation(
            1000, 500, 50, 0.20, size_bytes, 2000, 50.0, 30
        );
        let pct = result.available_content as f64 / result.total_content as f64 * 100.0;
        println!("║ Image size {}: {:.1}% overall availability                            ║",
                 size_name, pct);
    }
    println!("╚══════════════════════════════════════════════════════════════════════════╝");
    println!();
    ...
    for cache_mb in [100, 250, 500, 1000] {
        let result = run_availability_simulation(
            1000, cache_mb, 50, 0.20, 2_000_000, 2000, 50.0, 30
        );
        let pct = result.available_content as f64 / result.total_content as f64 * 100.0;
        println!("║ Cache {}MB: {:.1}% overall availability                                 ║",
                 cache_mb, pct);
    }
    println!("╚══════════════════════════════════════════════════════════════════════════╝");
}
