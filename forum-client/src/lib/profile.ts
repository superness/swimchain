...
/**
 * IMPORTANT: Privacy Transition Limitations
 *
 * When switching a profile from public to private:
 *
 * 1. NEW CONTENT CREATED
 *    - A new post is created with encrypted data (new content hash)
 *    - The new encrypted post becomes the "current" profile
 *
 * 2. OLD CONTENT PERSISTS
 *    - The old unencrypted post still exists on nodes that have it cached
 *    - Anyone who already saw the public profile has it locally
 *    - Content may eventually decay off active nodes
 *
 * 3. CHAIN DATA IS PERMANENT
 *    - Block data that references the old content hash is PERMANENT
 *    - Even if content decays, the reference exists forever in the chain
 *    - Someone could look at historical blocks, find the old hash, and
 *      request that content from any node/archive that still has it
 *
 * This is fundamental to blockchain/content-addressed systems:
 * - You cannot delete or modify existing content
 * - You can only create new content that supersedes it
 * - Historical data remains accessible to those who look for it
 *
 * Users should understand: "private profile" encrypts NEW data, but
 * does not erase previously published public data from the network.
 */
