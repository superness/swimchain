 ...
 */
export function ed25519PrivateToX25519(ed25519PrivateKey: Uint8Array): Uint8Array {
  ...
  // These indices are guaranteed to be valid for a 32-byte array
...
export function ed25519PublicToX25519(ed25519PublicKey: Uint8Array): Uint8Array {
  ...
  // The conversion uses: u = (1 + y) / (1 - y)
  // Where y is the Ed25519 point's y-coordinate
  // This is handled by @noble/curves internally when we compute shared secret

  // For now, we use a simplified approach:
  // Derive X25519 keypair from same seed and take the public key
  // This works because ed25519 and x25519 share the same underlying curve

  // Actually, @noble/curves provides this conversion via the Point class
  // But for simplicity, we'll compute the X25519 public key from private key
  // when we have access to it

  // For the recipient's public key (where we only have the public key),
  // we need to do the actual conversion
 ...
 */
export function x25519Unbox(
  boxedMessage: Uint8Array,
 ...
 */
export function encryptSpaceKeyForRecipient(
  spaceKey: Uint8Array,
 ...
 */
export function decryptSpaceKey(
  encryptedSpaceKey: Uint8Array,
