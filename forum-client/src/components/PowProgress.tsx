...
// Hash rate aligned with swimchain-wasm/src/pow.rs:271 default (500,000 h/s)
function useMiningEstimate(difficulty: number): { formatted: string } {
  return useMemo(() => {
    const expectedAttempts = Math.pow(2, difficulty);
    const hashRate = 500_000; // Aligned with WASM estimate_mining_time default
...
export function PowProgress({
  ...
  // After 95%, switch to indeterminate style to reduce user anxiety
  const expectedAttempts = Math.pow(2, difficulty);
  const rawProgress = (attempts / expectedAttempts) * 100;
  const progressPercent = Math.min(rawProgress, 95);
  const isIndeterminate = rawProgress >= 95;
  ...
  return (
    <div className="pow-progress" role="status" aria-live="polite">
      ...
      <div
        className={`pow-progress-bar${isIndeterminate ? ' pow-progress-bar--indeterminate' : ''}`}
        role="progressbar"
        aria-valuenow={isIndeterminate ? undefined : Math.round(progressPercent)}
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={isIndeterminate ? 'Mining in progress, almost done...' : `Mining progress: ${Math.round(progressPercent)}%`}
      >
        <div
