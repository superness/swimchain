 ...
 * leading zero BITS. The node validates bits (claim_sponsorship_offer in
 * src/rpc/methods.rs); this miner previously counted zero BYTES against the
 * same number, over-mining 8x and exhausting the attempt cap on any offer
 * with difficulty above ~24 — claims never completed and looked like a timeout.
 */
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        logger.error('[ClaimOffer] Error message:', err.message);
        logger.error('[ClaimOffer] Error stack:', err.stack);
