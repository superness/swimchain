...
export function ReportModal({ contentId, onClose }: ReportModalProps) {
  ...
  const handleReport = async () => {
    if (!selectedReason || !canSign || !publicKeyHex) return;

    const signFn = (message: Uint8Array) => sign(message);
  ...
  const handleDefend = async () => {
    if (!canSign || !publicKeyHex) return;
      ...
      <div className="report-modal" ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="modal-title">
        ...
        <div className="report-modal-actions">
          ...
          <button
            ...
            disabled={!selectedReason || !canSign}
          >
            Report
          </button>
        </div>
        ...
        {!canSign && (
          <p style={{ fontSize: '0.75rem', color: '#aaa', marginTop: '12px' }}>
