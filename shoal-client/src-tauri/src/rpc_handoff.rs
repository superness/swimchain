//! Copied from `trench-client/src-tauri/src/rpc_handoff.rs` (itself a verbatim copy of
//! `launcher-apps/app-shell/src-tauri/src/rpc_handoff.rs`): reads the node's own
//! `.rpc_addr` / `.cookie` handoff files from its data dir. These are written by the
//! node itself once RPC is actually listening, so — unlike computing the port from
//! `NodeManager::rpc_port()` before the node has necessarily bound it — this can never
//! hand back a stale or not-yet-live endpoint.
...
mod tests {
    ...
    fn reads_addr_and_cookie_into_endpoint() {
        let dir =
            std::env::temp_dir().join(format!("shoal-rpc-handoff-test-{}", std::process::id()));
    ...
    fn errors_when_node_not_running() {
        let dir = std::env::temp_dir().join(format!(
            "shoal-rpc-handoff-test-missing-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
