    ...
    "Swimchain benchmarks": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "b50327af86ad992e56577803f709678a78452963",
          "message": "fix(consensus): judge incoming block leader with its own chain's difficulty\n\nLeader-eligibility validation gathered recent block timestamps (for difficulty\nadjustment) from OUR canonical chain via get_root_hash_at_height, instead of\nfrom the ancestors of the block being validated. A block that extends a\ncompeting fork was therefore judged with the wrong difficulty, so a valid\nleader on a heavier chain was rejected as \"ineligible\" — permanently blocking\nreorg onto a chain that diverged below our tip (two nodes on different chains\ncould never agree on each other's block eligibility, so forks became sticky).\n\nWalk back from the block's actual parent (prev_root_hash) to collect the\ndifficulty timestamps, matching what the block's creator used. Observed on a\nmobile node stuck at height 30 while the network was at 40, rejecting every\ncanonical block as \"ineligible leader\".\n\nCo-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T10:08:47-04:00",
          "tree_id": "1ed8c4194a985b1f7cb2d0d95899f7e817b61919",
          "url": "https://github.com/superness/swimchain/commit/b50327af86ad992e56577803f709678a78452963"
        },
        "date": 1783954092546,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "86fe74d80feebecd245e7c7e6d022b2863c51a45",
          "message": "fix(blocks): sync block builder to canonical tip before each formation check\n\nRoot cause of the network-block formation stall: sync_chain_state() — which\nadvances the builder's current_height/prev_root_hash/prev_cumulative_pow to the\nchain tip — was only ever called once, at node startup (manager.rs). Blocks\nreceived from peers advance the chain store but never sync the builder. So the\nmoment a node falls one block behind a peer-formed block, builder.next_height()\npoints at a height that already exists, and the formation loop perpetually skips\n(\"block already exists at <stale height>\") — never forming again until restart.\nOn a quiet network every node eventually drifts a block behind and the chain\nwedges.\n\nFix: at the top of each formation-loop tick, sync the builder to the chain\nstore's current tip (idempotent — sync_chain_state only advances). next_height\nis then always correct and formation resumes on top of peer blocks. Explains the\n\"forms right after restart, then goes silent\" behavior and the deep-fork stalls.\n\nCo-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T10:49:46-04:00",
          "tree_id": "11fd7ac601c7b2fd6aa1a0add06719c7ebac20b0",
          "url": "https://github.com/superness/swimchain/commit/86fe74d80feebecd245e7c7e6d022b2863c51a45"
        },
        "date": 1783955845949,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "1085704983fe196f044df63c42d26689d9264abd",
          "message": "fix(consensus): validate block leader with the builder's EXACT eligibility inputs\n\nBlock-leader validation and the block builder disagreed on eligibility, so a\nnode formed a block it deemed eligible while every peer computed a different\nresult and rejected it as \"ineligible leader\" — permanent forks / stuck nodes\n(observed: local stuck at height 50 rejecting the seed's height-51 block).\n\nThree mismatches vs spawn_block_formation, now fixed in validation:\n  1. space_id: builder uses [0;16] (global); validation used the block's\n     first_space_id, changing the block seed and flipping eligibility.\n  2. window: builder uses 11 timestamps (heights tip-10..=tip); validation used 10.\n  3. order: builder gathers oldest->newest; validation gathered newest->first.\n\nValidation now uses [0;16], walks the block's own ancestors for the last 11\ntimestamps, and reverses to oldest->newest — matching the builder exactly while\nstill judging a block on the chain it extends (so reorg onto a heavier fork\nworks). Supersedes the partial fix in b50327af.\n\nCo-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T11:25:15-04:00",
          "tree_id": "5dccdf47f4e8d785a8e3ac6f05b007498d199160",
          "url": "https://github.com/superness/swimchain/commit/1085704983fe196f044df63c42d26689d9264abd"
        },
        "date": 1783957934965,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "e867351c3d23fdbfa9dbf24b8b92b099ccee1084",
          "message": "fix(consensus): dedup actions across content blocks within a formed block\n\nSecond facet of the testnet stall: a fresh chain diverged at h1/h2 because\nthe seed formed a root block containing the same action in two content\nblocks. A peer marks the action finalized while storing the first content\nblock, then skips the second as \"already-finalized\" (router path ~3905),\nso its copy of the block structurally diverges from the forming node's and\nthe chain stalls.\n\nThe mempool dedups by action hash on add (seen_actions), but that set is a\nbounded LRU: under a content-sync flood an action hash can be evicted while\nthe action is still pending under one thread_id, letting the same action be\nre-added under another. build_root_block now dedups by action hash across\nall content blocks of the block it forms, keeping the first occurrence in\ndeterministic (sorted space, thread, action) order so every node forms the\nidentical block. Complements the reorg re-add guards and the formation\nbackstop from 5164082e.\n\nCo-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T12:36:22-04:00",
          "tree_id": "ad651ceb0b201d7d62f98593dab1b032838eae93",
          "url": "https://github.com/superness/swimchain/commit/e867351c3d23fdbfa9dbf24b8b92b099ccee1084"
        },
        "date": 1783963347462,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "9d33199320e48125c582ede00e8a01f75b320220",
          "message": "fix(consensus): resync block builder down to chain tip after a rejected form\n\nThird facet of the stall: after the formation backstop (or CreateSpace\nvalidation) rejects a locally-formed block, the chain wedges at its current\nheight while the node logs \"Formed block (height=N+2)\" and peers reject those\nas \"too far ahead / invalid parent\".\n\nbuild_root_block advances current_height / prev_root_hash / prev_cumulative_pow\nthe instant it forms a block, before the caller decides whether to store it.\nWhen the caller rejects the block, the store stays put but the builder is left\none height ahead on a phantom parent. sync_chain_state only advances, so it can\nnever pull the builder back — every subsequent tick forms on the phantom parent.\n\nAdd BlockBuilder::reset_to_chain_tip (forces the tip state, regressing if the\nbuilder ran ahead) and call it at the top of the formation loop against the\nchain store's real tip, so a rejected form self-corrects instead of cascading.\nPre-existed the backstop (CreateSpace validation rejects the same way); the\nbackstop just surfaced it.\n\nCo-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T13:24:03-04:00",
          "tree_id": "ae4ae905d344b4cf741d2a010645834d9f9e1ee4",
          "url": "https://github.com/superness/swimchain/commit/9d33199320e48125c582ede00e8a01f75b320220"
        },
        "date": 1783966097799,
        "tool": "cargo",
        "benches": [
      ...
      },
      {
        "commit": {
          ...
          "distinct": true,
          "id": "dfd412dc65e16b20c775ee177c3c554481eb52fb",
          "message": "fix(consensus): judge leader eligibility at the block's quantized timestamp (F8)\n\nFreshly-syncing peers rejected the chain with \"created by ineligible leader\",\nstranding them below the tip — the F8 permanent-fork bug, distinct from the\ndouble-inclusion/desync fixes.\n\nRoot cause: the eligibility threshold expands with elapsed-since-parent, but\nformation judged is_eligible() at raw wall-clock `now` while build_root_block\nstores a timestamp quantized down to a 10s window. A peer re-validating the\nsynced block judges eligibility at that earlier quantized timestamp — a smaller\nelapsed, hence a stricter threshold — so a block marginally eligible at raw\n`now` fails validation. Real-time peers hide it: they pre-store the header via\nBLOCK_ANNOUNCE and skip re-validation on content backfill (known_header_backfill);\nonly nodes that full-validate a synced block (fresh/batch joiners) hit it.\n\nFix: quantize `now` to the 10s window BEFORE the eligibility check and reuse\nthat same value for build_root_block, so formation, the stored timestamp, and\npeer validation all judge the identical instant. Test asserts eligibility is\nmonotonic in time and can flip within one quantum (the straddle that rejected\nvalid blocks).\n\nNote: blocks formed before this fix carry raw-judged timestamps and remain\nunvalidatable by fresh peers — a chain reset is needed so every block is\nclean-formed.\n\nCo-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>",
          "timestamp": "2026-07-13T14:19:39-04:00",
          "tree_id": "08f020e305dbbd99f3d872d07ccde160f82422c2",
          "url": "https://github.com/superness/swimchain/commit/dfd412dc65e16b20c775ee177c3c554481eb52fb"
        },
        "date": 1783968375626,
        "tool": "cargo",
        "benches": [
